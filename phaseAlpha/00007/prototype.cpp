...
int main(int argc, char* argv[])
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Draw ground
        mat4 groundWorldMatrix = translate(mat4(1.0f), vec3(0.0f, -0.01f, 0.0f)) * scale(mat4(1.0f), vec3(10.0f, 0.05f, 10.0f));
        GLuint worldMatrixLocation = glGetUniformLocation(shaderProgram, "worldMatrix");
        glUniformMatrix4fv(worldMatrixLocation, 1, GL_FALSE, &groundWorldMatrix[0][0]);
        glDrawArrays(GL_TRIANGLES, 0, 36);


           mat4 yidaCube = translate(mat4(1.0f), vec3(5.0f, 5.0f, 5.0f));
        glUniformMatrix4fv(worldMatrixLocation, 1, GL_FALSE, value_ptr(mat4(1.0f)));
...
int createVertexArrayObject()
{
    ...
    vec3 vertexArray[] = {  // position,                            color
        ...
        vec3(0.5f, 0.5f, 0.5f), vec3(0.65f, 0.19f, 0.36f), // pink overdrive
        vec3(0.5f, 0.5f,-0.5f), vec3(0.65f, 0.19f, 0.37f),
        vec3(-0.5f, 0.5f,-0.5f), vec3(0.65f, 0.19f, 0.38f),

        vec3(0.5f, 0.5f, 0.5f), vec3(0.65f, 0.19f, 0.39f),
        vec3(-0.5f, 0.5f,-0.5f), vec3(0.65f, 0.19f, 0.40f),
        vec3(-0.5f, 0.5f, 0.5f), vec3(0.65f, 0.19f, 0.41f)
    };
...
bool initContext() {     // Initialize GLFW and OpenGL version
    ...
    window = glfwCreateWindow(800, 600, "Eden's Infinity", NULL, NULL);
    if (window == NULL)
    {
